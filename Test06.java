...
public class Test06 {
	public static void main(String[] args) {
		...
		HashMap<Integer,String> allProvinces = new HashMap<Integer,String>();
		
		//key是省份的编号
		//value是该省的城市
		HashMap<Integer,List<String>> all = new HashMap<Integer,List<String>>();
		
		//第一步按照;拆分，各个省的信息就分开了
		String[] split = str.split(";");
		
		//按照:拆分，:左边是省的名称，右边是该省的城市
		for (int i = 0; i < split.length; i++) {
			String[] provinceAndCity = split[i].split(":");
			//provinceAndCity[0]是省份
			//provinceAndCity[1]是城市
			String province = provinceAndCity[0].replace("'", "");
			allProvinces.put(i+1, province);
			
//			provinceAndCity[1]去掉[,],',空格
			String temp = provinceAndCity[1].replaceAll("\\[|\\]| |'", "");
//			provinceAndCity[1]按照,拆分
			String[] citys = temp.split(",");
			all.put(i+1, Arrays.asList(citys));
		}
		...
		List<String> list = all.get(select);
